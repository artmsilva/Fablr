...
| **Last Update** | `2025-11-29 03:33 EST` |
...
- Production blank page fixed by making the base tag relative (\`<base href="./">\`), ensuring hashed assets resolve correctly when deployed under a subdirectory; static export rebuilt via \`npm run build\`. Need to restart MCP DevTools helper if we want the original failing trace.
